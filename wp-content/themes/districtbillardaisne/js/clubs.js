...
/*
MediaQ.add(1200, () =>{
    responsiveCoordonnes(true)
}, '<')
MediaQ.add(1199, () =>{
    responsiveCoordonnes()
}, '>')
*/

function responsiveCoordonnes(bool){
    if(bool === undefined)
        bool = false
    let coord = document.querySelector('.coordonnees')
    let parentCoord = document.querySelector('#info .row >.col:nth-of-type(1)')

    if(bool){
        let newParent = document.querySelector('#info .row >.col:nth-of-type(2)')
        //newParent.insertBefore(coord, newParent.firstChild);
        newParent.appendChild(coord)
    } else {
        console.log(parentCoord)
        parentCoord.appendChild(coord)
    }
}
